...
import { WorldBankProcNoticesEnvelopeSchema } from '../schemas.js';
...
export async function fetchWorldBankProcurementNotices(options) {
    ...
    while (results.length < options.maxItems && os < total && pagesFetched < HARD_PAGE_FETCH_CAP) {
        ...
        for (const raw of batch) {
            if (countryFilter && !countryFilter.has(raw.project_ctry_name ?? ''))
                continue;
            if (noticeTypeFilter && !noticeTypeFilter.has(raw.notice_type ?? ''))
                continue;
            results.push(raw);
...
//# sourceMappingURL=worldBankProcurementNotices.js.map