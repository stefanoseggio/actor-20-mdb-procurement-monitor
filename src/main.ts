...
import { ActorInputSchema, type UnifiedRecord } from './schemas.js';
...
async function pushRecord(record: UnifiedRecord, eventName: string): Promise<boolean> {
...
async function run(): Promise<void> {
    ...
    try {
        if (!stopped && input.sources.includes('worldBankProcurementNotices')) {
            ...
            const entriesThisRun: Record<string, RecordEntry> = {};
            for (const raw of rawNotices) {
                ...
                if (input.dateRange) {
                    ...
                    if (!isWithinDateRange(noticeDate, input.dateRange, now)) continue;
                }
        ...
        if (!stopped && input.sources.includes('worldBankDebarredFirms')) {
            ...
            if (extraction.degraded) {
                ...
                const notice = buildDebarredFirmsDegradedNotice(
                    scrapedAt,
                    extraction.degradedReason ?? 'unknown extraction failure',
                );
